...
from seq_2_seq_metrics import word_accuracy, edit_distance, neg_edit_distance
...
from constants import ENCODING_NONE, METRIC_ACCURACY, METRIC_F1, METRIC_PRECISION, METRIC_RECALL, TASK_TYPE_AM, \
    ...
    METRIC_MEDIAN_EDIT_DISTANCE, METRIC_NEG_AVG_EDIT_DISTANCE, METRIC_NEG_MEDIAN_EDIT_DISTANCE
...
class ResultList(list):
    ...
    def get_true_and_pred(self):
        ...
        for _, pred, gold, _, _, _, _ in self:
    ...
    def confusion_matrix(self):
        ...
        return confusion_matrix(y_true, y_pred, labels=self.idx_2_label.values())
    ...
    def print_confusion_matrix(self, matrix=None):
        ...
        labels = self.idx_2_label.values()
    ...
    def precision(self, correct_bio_errors="No"):
        ...
        if self.task is None or self.task.encoding == ENCODING_NONE:
            ...
            y_true, y_pred = self.get_true_and_pred()
            return precision_score(y_true, y_pred, labels=self.idx_2_label.values(), average="macro")
        else:
            ...
            # Using scikit learn implementation instead
            # Flatten list of tokens before

            y_true = [label for sentence in y_true for label in sentence]
    ...
    def recall(self, correct_bio_errors="No"):
        ...
        if self.task is None or self.task.encoding == ENCODING_NONE:
            # Not BIO, IOB or IOBES
            y_true, y_pred = self.get_true_and_pred()
            return recall_score(y_true, y_pred, labels=self.idx_2_label.values(), average="macro")
        else:
            y_true, y_pred = self.get_true_and_pred_sentences(word=False)
            y_true, y_pred = pre_process(
                y_pred,
    ...
    def f1(self, correct_bio_errors="No"):
        ...
        if self.task is None or self.task.encoding == ENCODING_NONE:
            # Not BIO, IOB or IOBES
            y_true, y_pred = self.get_true_and_pred()
            return f1_score(y_true, y_pred, labels=self.idx_2_label.values(), average="macro")
        else:
            y_true, y_pred = self.get_true_and_pred_sentences(word=False)
            y_true, y_pred = pre_process(
                y_pred,
    ...
    def neg_edit_distance(self, mode="avg"):
        """
        Calculate the negative edit distance.
        Use this only for seq2seq tasks.
        ...
        Returns:
            float: negated average edit distance
        """
        assert mode in ["avg", "median"]

        y_true, y_pred = self.get_true_and_pred_sentences(word=True)
        return neg_edit_distance(y_pred, y_true, mode)
    ...
    def compute_metric_by_name(self, metric_name):
        ...
        if metric_name == METRIC_ACCURACY:
            ...
            return self.argmin_relations(ratio=0.999)
        ...
        elif metric_name == METRIC_NEG_AVG_EDIT_DISTANCE:
            return self.neg_edit_distance(mode="avg")
        elif metric_name == METRIC_NEG_MEDIAN_EDIT_DISTANCE:
            return self.neg_edit_distance(mode="median")
        else:
